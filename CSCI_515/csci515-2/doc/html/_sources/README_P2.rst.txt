...
Symbols and symbol tables are introduced to manage the declaration, access, and scope of variables that are created in GPL.
...
8.  Write the ``Scope_manager`` class. This class is a *singleton* class that ensures no more than one ``Scope_manager`` object exists in the application. See ``sample_singleton.cpp`` for an example of how to structure a class as a singleton. Ensure that ``sample_singleton.cpp`` s moved out of the project directory or the ``Makefile`` will erroneously add it to the project build. Use a ``std::vector`` as the private data to manage multiple symbol tables::
    ...
    ``void pop_table()`` 
        If more than one table is on the vector remove the table on the back of the vector. There must always be at least one table in the vector. Throw an exception if there is an attemt to pop it off::

          #include<stdexcept>
          throw std::underflow_error("Can't pop global symbol table");

    ``bool add_to_current_scope(std::shared_ptr<Symbol> sym)`` 
        Add ``sym`` to the last/back table in the vector via a call to ``Symbol_table::insert``. Return the value that ``Symbol_table::insert`` returns.
    ...
    * ``Scope_manager``'s default constructor should push an initial ``Symbol_table`` object onto the vector, representing the global namespace (which holds global variables).

|

9.  Write a test program to ensure ``Scope_manager`` is bug-free. Run the test code through ``valgrind`` to ensure it is free of memory leaks. A small modification to the previous test program should be sufficient::
...
14. At this point the project can be built (``make``) and tested (``../../bin/gt``). Several tests will fail.  
    ...
    If the default case is entered GPL will immediately abort. Correct code will not enter the default case. An abort allows for prompt debugging of the issue.
...
15. For the ``variable_declaration:`` production, write an action for each of the two rules. The action for the first rule will allocate and initialize individual variables. The action for the second rule will allocate and initialize arrays. For simplicity at this point, each individual variable and each element of an array should be initialized using hard-code values::
    ...
    The code used to test ``Symbol_table`` and ``Scope_manager`` can provide clues on how to do this. Note that array elements will have to be initialized using a loop.

|

16. Running the project tests reveals that several fail because required error-checking hasn't been added yet. Use ``vde`` to check error output, inspect the content of the ``tests/t###.gpl`` files directly to see why the test's GPL code generates an error, and consult ``error.cpp`` for the enumeration that corresponds to the expected error. Example of error-handling code:
...
* Added ``.. only:: latex`` boxes following ``.. raw:: html`` boxes.
* Added colons to some mentions of specific productions.
