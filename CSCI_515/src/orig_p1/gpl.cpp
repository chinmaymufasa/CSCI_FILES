...
int main(int argc, char **argv)
{
  ...
  // if -DGRAPHICS was specified when compiling gpl.cpp then include this code
    ...
    symbol = Scope_manager::instance().lookup("window_x");
    if (symbol != nullptr)
    {
    ...
    symbol = Scope_manager::instance().lookup("window_y");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
    ...
    symbol = Scope_manager::instance().lookup("window_width");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
    ...
    symbol = Scope_manager::instance().lookup("window_height");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
    ...
    symbol = Scope_manager::instance().lookup("window_red");
    if (symbol != nullptr)
    {
    ...
    symbol = Scope_manager::instance().lookup("window_green");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != DOUBLE)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
    ...
    symbol = Scope_manager::instance().lookup("window_blue");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != DOUBLE)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
    ...
    symbol = Scope_manager::instance().lookup("window_title");
    if (symbol != nullptr)
    {
    ...
    symbol = Scope_manager::instance().lookup("animation_speed");
    if (symbol != nullptr)
    {
      if (symbol->get_type() != INT)
        Error::error(Error::INVALID_TYPE_FOR_RESERVED_VARIABLE,
